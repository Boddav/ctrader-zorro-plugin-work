...
### MLDATACOLLECTION v7 (7 asset) — PF 0.73 tanár, PF 0.84 diák FAILED
- 7 × 12 = 84 optimize param → WFO NEM konvergál
- "no EUR/USD:SMA parameters at walk 9" hiba
...
## Kód Audit Eredmények (2026-03-07)
- **Current_State eltérés**: tanár `Balance-InitialBalance`, diák `Equity-Capital` → kis shift
- **CH LifeTime**: tanár optimize(), diák hardcode 0 → /predict feleslegesen prediktálja
- **ATR-alapú küszöbök**: PIP helyett ATR (XAU/USD fix) — MLDRIVEN, v8, v8_ICT
- **Margin safety**: safeToOpen flag (margin level < 200% → nem nyit, exit fut)
- **MLDRIVEN_Assets.csv**: 7 asset konfigurálható, de 3 aktív (CSV-ből)
- See [lessons.md](lessons.md) for workflow rules + never-do-again list
