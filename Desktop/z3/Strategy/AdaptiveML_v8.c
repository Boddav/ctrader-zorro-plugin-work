...
static int smartHold[14];                  // smart exit hold flags: 0-6 long, 7-13 short
static int tradeBarL[NUM_ASSETS];          // bar number at long entry
static int tradeBarS[NUM_ASSETS];          // bar number at short entry

// Phantom equity curve filter — per-asset algo() szinten működik
...
function run()
{
	...
	// 1. [Train] → RULES (PERCEPTRON) → _ml.c + flag
	// 2. [Train] → PARAMETERS (optimize) → .par + flag törlés
	// [Test]: mindkettőt betölti
	int trainStep = 1;
	if(Train)
	{
		if(file_date("Data\\AdaptiveML_v8_step1_done"))
			trainStep = 2;
		if(trainStep == 1)
		{
	...
	var XGBThresh = slider(3, 50, 40, 80, "XGB Thr", "XGBoost prob% threshold");
	int UsePhantom = slider(4, 1, 0, 1, "Phantom", "Equity curve filter 0=off 1=on");
		...
		if(!Train)
		{
			...
			for(open_trades)
			{
				...
				if(TradeIsLong)
				{
					tSide = 1;
					holdIdx = aIdx;
					int barsHeld = Bar - tradeBarL[aIdx];
					// Channel exit only after minimum 5 bars
					if(barsHeld >= 5 && price > EntryHigh) chExit = 1;
					if(barsHeld == 20) chExit = 2;
				}
				if(TradeIsShort)
				{
					tSide = -1;
					holdIdx = aIdx + NUM_ASSETS;
					int barsHeld = Bar - tradeBarS[aIdx];
					// Channel exit only after minimum 5 bars
					if(barsHeld >= 5 && price < EntryLow) chExit = 1;
					if(barsHeld == 20) chExit = 2;
				}

				if(smartHold[holdIdx] > 0)
				{
					smartHold[holdIdx] = smartHold[holdIdx] + 1;
					int momDead = 0;
					if(tSide == 1 && Close[0] <= Close[1]) momDead = 1;
					if(tSide == -1 && Close[0] >= Close[1]) momDead = 1;
					if(smartHold[holdIdx] > 10) momDead = 1;
				...
				{
					int mom = 0;
					if(tSide == 1 && Close[0] > Close[1]) mom = 1;
					if(tSide == -1 && Close[0] < Close[1]) mom = 1;

					int doHold = 0;

					if(mom && (is(TRADEMODE) || is(TESTMODE)))
					{
						char cArr[600];
						char tmp[32];
						int k;
						...
						{
							if(k < 19) strcat(cArr, ",");
							sprintf(tmp, "%.6f", Close[k]);
							strcat(cArr, tmp);
						}
						strcat(cArr, "]");

						char pBuf[1200];
						sprintf(pBuf, "{\"asset\":\"%s\",\"side\":%d,\"atr\":%.6f,\"closes\":%s,\"features\":[%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f,%.4f]}", assetCode, tSide, h1atr, cArr, Sigs[0],Sigs[1],Sigs[2],Sigs[3],Sigs[4],Sigs[5],Sigs[6],Sigs[7],Sigs[8],Sigs[9]);

						string resp = http_transfer("http://127.0.0.1:5000/hold_or_exit", pBuf);
						if(resp)
						{
							if(strstr(resp, "\"hold\""))
								doHold = 1;
	...
	if(is(EXITRUN))
	{
		...
		{
			file_write("Data\\AdaptiveML_v8_step1_done", "1", 0);
